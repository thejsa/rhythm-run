...
SceneGameplayEnd::SceneGameplayEnd(SceneManager& a_sceneManager,
...
void SceneGameplayEnd::onCreate()
{
    // Load sprite sheet
    eprintf("OnCreate\n");

    spriteSheet = C2D_SpriteSheetLoad("romfs:/gfx/SceneGameplayEnd.t3x");
    if (!spriteSheet) {
    ...
    topScreenLabel = std::make_shared<Label>("Level finished!", 200, 100,
...
void SceneGameplayEnd::onFocus()
{
    // eprintf("Focus\n");
    // Reset duration timer
    // durationElapsed = 0.0f;
    eprintf("the score: %ld\n", scoreDisplay);
    eprintf("This: %x\n", this);
    ...
    char scoreLabelBuf[256];
    int scoreLabelSize = sprintf(scoreLabelBuf, "Level finished!\nScore: %ld", scoreDisplay);
    topScreenLabel->setText(scoreLabelBuf, scoreLabelSize);

    // sample.load("romfs:/sample.wav");
    // soloud.play(sample);
}

void SceneGameplayEnd::onBlur()
{
    if (opusFile) {
        audioManager.pause();
        eprintf("pause audio\n");
...
void SceneGameplayEnd::onDestroy()
{
    // soloud.deinit();
...
// void SceneGameplayEnd::setNextSceneId(unsigned int a_id)
// {
// 	eprintf("%u\n", a_id);
// 	nextSceneId = a_id;
// }
void SceneGameplayEnd::setScore(long a_score)
{
    eprintf("Score Set to %ld\n", a_score);
    eprintf("This: %x\n", this);
    scoreDisplay = a_score;
}

void SceneGameplayEnd::processInput()
{
...
void SceneGameplayEnd::update(float a_timeDelta)
{
    char scoreLabelBuf[256];
    // int scoreLabelSize = sprintf(scoreLabelBuf, "Level finished!\nScore: %ld\n", scoreDisplay);
    int scoreLabelSize = sprintf(scoreLabelBuf, "Level finished!", scoreDisplay);
    topScreenLabel->setText(scoreLabelBuf, scoreLabelSize);
...
void SceneGameplayEnd::draw(RenderWindow& a_renderWindowUpper, RenderWindow& a_renderWindowLower)
{
